(function () {
    ...
    switch (key) {
      ...
      default:
        if (!isSvg && key != "list" && key in node) {
          ...
          node.setAttribute(key, _typeof(nextValue) == "object" ? JSON.stringify(nextValue) : nextValue);
        }
  ...
  function mapChildren(children) {
    ...
    if (isArray(children)) {
      ...
      if (vnode != null && _typeof(vnode) == "object") {
  ...
  function render(vnode, node) {
    ...
    if (vnode != null && _typeof(vnode) == "object" && vnode.nodeType != NODE_HOST) {
  ...
  function setAttr(node, attr, value) {
    if (value == null) {
      ...
      node.setAttribute(attr, _typeof(value) == "object" ? JSON.stringify(value) : value);
    }
  }
  ...
  function dispatchEvent(node, type, customEventInit) {
    node.dispatchEvent(new CustomEvent(type, _typeof(customEventInit) == "object" ? customEventInit : null));
  }
